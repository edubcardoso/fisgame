...
# DEFINE A GAME CLASS
class Game:
    def __init__(self, fisga_group_, drink_group_, fish_group):
    ...
    def check_collisions(self):
        if pygame.sprite.groupcollide(self.fish_group, self.drink_group, True, True):
            self.score += 1
            self.score_sound.play()


class Fish(pygame.sprite.Sprite):
    def __init__(self, x, y, fish_group):
        super().__init__()
        self.image = pygame.image.load('images/fish_up2.png')
    ...
    def update(self):
        # MOVE THE FISH AFTER SHOOTING, ONLY UP
        self.rect.y -= self.velocity
        # DELETE BONE WHEN IT REACHES THE TOP OF THE BOX
...
class Fisga(pygame.sprite.Sprite):
    ...
    def fire(self):
        # FIRE THE FISH
        # if len(self.fish_group) < 3:  # ONLY 3 FISHES
        Fish(self.rect.centerx, self.rect.top, self.fish_group)
...
# CREATE FISH GROUP
fish_group_ = pygame.sprite.Group()
...
fisga_ = Fisga((WINDOW_WIDTH/2)-50, 510, fish_group_)
# ADD FISGA TO THE GROUP
fisga_group.add(fisga_)

# CREATE GAME OBJECT
our_game = Game(fisga_, drink_group, fish_group_)
